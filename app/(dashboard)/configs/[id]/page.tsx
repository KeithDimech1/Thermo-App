...
import { getAllConfigs, getConfigById } from '@/lib/db/queries';

// Generate static params for all config pages at build time
export async function generateStaticParams() {
  try {
    // Get curated configs only for build performance (limit to reasonable number)
    const { data } = await getAllConfigs({ dataset: 'curated' }, 1000, 0);
    return data.map((config) => ({
      id: config.config_id.toString(),
    }));
  } catch (error) {
    console.error('Error generating static params for configs:', error);
    return [];
  }
}
...
// Fetch config data directly from database for static generation
async function fetchConfigData(id: string): Promise<TestConfigDetail | null> {
  try {
    ...
    return config as TestConfigDetail | null;
  } catch (error) {
        ...
        <CardContent className="space-y-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-gray-600">
                {config.quality_rating === 'excellent' &&
                  'This configuration demonstrates excellent precision with ≥95% of measurements achieving CV <10%. Suitable for critical diagnostic applications.'}
          ...
          <div className="pt-4 border-t border-gray-200">
            <div className="flex items-start gap-3 text-sm">
              <div className="flex-shrink-0 text-blue-600 mt-0.5">ℹ</div>
              <div className="text-gray-600">
                <span className="font-medium text-gray-900">Data Confidence: </span>
                {(config.events_examined || totalEvents) >= 50 && 'High confidence - based on ≥50 QC events'}
