...
async function extractTableAttempt(
  ...
  // Get CSV statistics (will be recalculated after empty column removal)
  let stats = getCSVStats(csvData);
  console.log(`[Extract] Stats: ${stats.totalRows} rows, ${stats.totalColumns} columns, ${stats.completeness.toFixed(1)}% complete`);

  // ═══════════════════════════════════════════════════════════════════
  // POST-PROCESSING: Remove completely empty columns
  // ═══════════════════════════════════════════════════════════════════
  // Claude sometimes hallucinates extra columns that are 100% empty.
  // Remove these before validation to handle overcorrection gracefully.
  const emptyColumnsBefore = stats.fieldStats.filter(f => f.fillRate === 0);
  if (emptyColumnsBefore.length > 0) {
    const emptyColumnNames = emptyColumnsBefore.map(f => f.field);
    console.log(`[Extract] Found ${emptyColumnsBefore.length} completely empty columns: ${emptyColumnNames.join(', ')}`);
    console.log(`[Extract] Removing empty columns (likely hallucinated by Claude)...`);

    // Remove empty columns from all rows
    csvData = csvData.map(row => {
      const cleanedRow: Record<string, string> = {};
      for (const [key, value] of Object.entries(row)) {
        if (!emptyColumnNames.includes(key)) {
    ...
    stats = getCSVStats(csvData);
    console.log(`[Extract] After cleanup: ${stats.totalRows} rows, ${stats.totalColumns} columns (removed ${emptyColumnsBefore.length} empty columns)`);
  }
